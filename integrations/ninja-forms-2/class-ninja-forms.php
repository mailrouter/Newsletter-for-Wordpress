...
/**
 * Class NL4WP_Ninja_Forms_v2_Integration
 *
 * @ignore
 */
class NL4WP_Ninja_Forms_v2_Integration extends NL4WP_Integration {
	...
	public function register_field() {
		$args = array(
			'name' => __( 'Newsletter', 'ninja-forms' ),
			'edit_function' => '',
		...
		ninja_forms_register_field( 'nl4wp-subscribe', $args );
	}
	...
	public function process( $id, $value ) {
		...
		foreach( $fields as $field_id => $field_value ) {
			...
			// try admin label for "nl4wp-" prefixed fields, otherwise use general label
			$label = $ninja_forms_processing->get_field_setting( $field_id, 'admin_label' );
			if( empty( $label ) || stripos( $label, 'nl4wp-' ) !== 0 ) {
		...
		// guess newsletter variables
		$parser = new NL4WP_Field_Guesser( $pretty );
