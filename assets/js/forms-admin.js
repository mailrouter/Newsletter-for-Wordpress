...
var FieldFactory = function FieldFactory(fields, i18n) {
    ...
    function registerMergeField(mergeField) {
        ...
        if (data.type !== 'address') {
            ...
            register(category, { name: data.name + '[addr1]', type: 'text', newsletterType: 'address', title: i18n.streetAddress });
            register(category, { name: data.name + '[city]', type: 'text', newsletterType: 'address', title: i18n.city });
            register(category, { name: data.name + '[state]', type: 'text', newsletterType: 'address', title: i18n.state });
            register(category, { name: data.name + '[zip]', type: 'text', newsletterType: 'address', title: i18n.zip });
            register(category, { name: data.name + '[country]', type: 'select', newsletterType: 'address', title: i18n.country, choices: nl4wp_vars.countries });
        }
...
function init(editor, fields) {
    ...
    var groupingsNotice = function groupingsNotice() {
        var text = "Your form contains old style <code>GROUPINGS</code> fields. <br /><br />Please remove these fields from your form and then re-add them through the available field buttons to make sure your data is getting through to Newsletter correctly.";
    ...
    var requiredFieldsNotice = function requiredFieldsNotice() {
        ...
        var text = '<strong>Heads up!</strong> Your form is missing list fields that are required in Newsletter. Either add these fields to your form or mark them as optional in Newsletter.';
...
// vars
var textareaElement = document.getElementById('nl4wp-form-content');
...
m.mount(document.getElementById('nl4wp-field-wizard'), fieldHelper);
