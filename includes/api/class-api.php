...
/**
 * Takes care of requests to the Newsletter API (version 2.0, deprecated)
 *
 * @access public
 * @uses WP_HTTP
 * @since 1.0
...
class NL4WP_API {
	...
	private function show_connection_error( $message ) {
		$message .= '<br /><br />' . sprintf( '<a href="%s">' . __( 'Read more about common connectivity issues.', 'newsletter-for-wp' ) . '</a>', 'https://kb.mc4wp.com/solving-connectivity-issues/#utm_source=wp-plugin&utm_medium=newsletter-for-wp&utm_campaign=settings-notice' );
	...
	public function call( $method, array $data = array() ) {
		...
		if( empty( $this->api_key ) ) {
			$this->error_message = "Missing Newsletter API key.";
			$this->error_code = 001;
			return false;
		}
	...
	private function get_headers() {
		...
		$headers = array(
            ...
            'User-Agent' => 'nl4wp/' . NL4WP_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' )
        );
	...
	private function parse_response( $response ) {
		...
		if( is_wp_error( $response ) ) {
			throw new Exception( 'Error connecting to Newsletter. ' . $response->get_error_message(), (int) $response->get_error_code() );
		}
		...
		if( $code !== 200 ) {
			$message = sprintf( 'The Newsletter API server returned the following response: <em>%s %s</em>.', $code, $message );
