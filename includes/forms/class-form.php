 ...
 *
 * To get a form instance, use `nl4wp_get_form( $id );` where `$id` is the post ID.
 *
 * @access public
 * @since 3.0
 */
class NL4WP_Form {
    ...
    public static function get_instance( $post = 0 ) {
        ...
        if( $post instanceof WP_Post ) {
            ...
            if( empty( $post_id ) ) {
                $post_id = (int) get_option( 'nl4wp_default_form_id', 0 );
            }
        }
        ...
        // check post object
        if( ! is_object( $post ) || ! isset( $post->post_type ) || $post->post_type !== 'nl4wp-form' ) {
            $message = sprintf( __( 'There is no form with ID %d, perhaps it was deleted?', 'newsletter-for-wp' ), $post_id );
            throw new Exception( $message );
        }

        $post_meta = get_post_meta( $post_id );
        $form = new NL4WP_Form( $post_id, $post, $post_meta );
     ...
     *
     * @see NL4WP_Form_Element::get_response_html()
     *
     * @return string
     */
     ...
     * @param array $config
     * @return NL4WP_Form_element
     */
    public function get_element( $element_id = 'nl4wp-form', $config = array() ) {
        return new NL4WP_Form_Element( $this, $element_id, $config );
    }

    /**
     * Get HTML string for this form.
     *
     * If you want to output a form, use `nl4wp_show_form` instead as it.
     *
     * @param string $element_id
     * @param array $config
     *
     * @return string
     */
    public function get_html( $element_id = 'nl4wp-form', array $config = array() ) {
    ...
    protected function load_settings() {
        ...
        if( ! $defaults ) {
            $defaults = include NL4WP_PLUGIN_DIR . 'config/default-form-settings.php';
        }
        ...
        if( ! empty( $this->post_meta['_nl4wp_settings'] ) ) {
            $meta = $this->post_meta['_nl4wp_settings'][0];
         ...
         */
        $settings = (array) apply_filters( 'nl4wp_form_settings', $settings, $form );
    ...
    protected function load_messages() {
        ...
        if( ! $default_messages ) {
            $default_messages = include NL4WP_PLUGIN_DIR . 'config/default-form-messages.php';
        }
         ...
         * @param NL4WP_Form $form
         */
        $messages = (array) apply_filters( 'nl4wp_form_messages', $messages, $form );
        ...
        foreach( $messages as $key => $message ) {
            ...
            $message = new NL4WP_Form_Message( $text, $type );
    ...
    public function __toString() {
        return nl4wp_show_form( $this->ID, array(), false );
    }
    ...
    public function get_redirect_url() {
         ...
         * @param NL4WP_Form $form
         */
        $url = (string) apply_filters( 'nl4wp_form_redirect_url', $url, $form );
        return $url;
    }
    ...
    public function validate() {
        ...
        // validate config
        $validator = new NL4WP_Validator( $this->config );
        $validator->add_rule( 'lists', 'not_empty', 'no_lists_selected' );
        $valid = $validator->validate();

        // validate internal fields
        if( $valid ) {
            $validator = new NL4WP_Validator( $this->raw_data );
            $validator->add_rule( '_nl4wp_timestamp', 'range', 'spam', array( 'max' => time() - 2 ) );
            $validator->add_rule( '_nl4wp_honeypot', 'empty', 'spam' );
            $valid = $validator->validate();

            // validate actual (visible) fields
            if( $valid ) {
                $validator = new NL4WP_Validator( $this->data );
         ...
         * @param NL4WP_Form $form
         */
        $errors = (array) apply_filters( 'nl4wp_form_errors', $errors, $form );

        /**
         * @ignore
         * @deprecated 3.0 Use `nl4wp_form_errors` instead
         */
        $form_validity = apply_filters( 'nl4wp_valid_form_request', true, $this->data );
     ...
     *
     * @see NL4WP_Form::validate
     * @param NL4WP_Request $request
     * @return void
     */
    public function handle_request( NL4WP_Request $request ) {
        ...
        $map = array(
            '_nl4wp_lists' => 'lists',
            '_nl4wp_action' => 'action',
            '_nl4wp_form_element_id' => 'element_id',
            '_nl4wp_email_type' => 'email_type'
        );
     ...
     * @return array
     */
    protected function parse_request_data( NL4WP_Request $request ) {
        $form = $this;
         ...
         * @param NL4WP_Form $form The form instance.
         */
        $ignored_field_names = apply_filters( 'nl4wp_form_ignored_field_names', $ignored_field_names, $form );
    ...
    /**
     * Get Newsletter lists this form subscribes to
     *
     * @return array
     */
    public function get_lists() {
        ...
        /**
         * Filters Newsletter lists new subscribers should be added to.
         *
         * @param array $lists
         */
        ...
        /**
         * Filters Newsletter lists new subscribers coming from this form should be added to.
         *
         * @param array $lists
         * @param NL4WP_Form $form
         */
        $lists = (array) apply_filters( 'nl4wp_form_lists', $lists, $form );
    ...
    public function get_data() {
         ...
         * @param NL4WP_Form $form
         */
        $data = apply_filters( 'nl4wp_form_data', $data, $form );
    ...
    public function get_required_fields() {
         ...
         *
         * - All fields which are required for the selected Newsletter lists
         * - All fields in the form with a `required` attribute.
         *
         * @param array $required_fields
         * @param NL4WP_Form $form
         */
        $required_fields = (array) apply_filters( 'nl4wp_form_required_fields', $required_fields, $form );

        return $required_fields;
    }

    /**
     * Get "email_type" setting for new Newsletter subscribers added by this form.
     *
     * @return string
     */
