...
class NL4WP_Forms_Admin {
	...
	public function add_hooks() {
		...
		add_action( 'nl4wp_admin_preview_form', array( $this, 'prepare_form_preview' ) );
	...
	public function process_save_form( ) {
		...
		$previewer = new NL4WP_Form_Previewer( $form_id );

		$this->messages->flash( __( "<strong>Success!</strong> Form successfully saved.", 'newsletter-for-wp' ) . sprintf( ' <a href="%s">', esc_attr( $previewer->get_preview_url() ) ) . __( 'Preview form', 'newsletter-for-wp' ) . '</a>' );
	}
	...
	public function prepare_form_preview() {
		$form_id = (int) $_POST['nl4wp_form_id'];
		$preview_id = (int) get_option( 'nl4wp_form_preview_id', 0 );
		...
		if( $real_preview_id != $preview_id ) {
			update_option( 'nl4wp_form_preview_id', $real_preview_id, false );
		}

		// redirect to preview
		$previewer = new NL4WP_Form_Previewer( $form_id, $real_preview_id );
