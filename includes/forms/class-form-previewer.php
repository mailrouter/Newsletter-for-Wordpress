...
class NL4WP_Form_Previewer {
	 ...
	 * @const string
	 */
	const PAGE_SLUG = 'nl4wp-form-preview';
	...
	public static function init() {
		...
		define( 'NL4WP_FORM_IS_PREVIEW', true );

		$form_id        = (int) $_GET[ 'form_id' ];
		$preview_id     = isset( $_GET['preview_id'] ) ? (int) $_GET['preview_id'] : 0;
		$instance = new self( $form_id, $preview_id );

		add_filter( 'nl4wp_form_stylesheets', array( $instance, 'set_stylesheet' ) );
		add_filter( 'nl4wp_form_css_classes', array( $instance, 'add_css_class' ), 10, 2 );
	...
	public function __construct( $form_id, $preview_id = 0 ) {
		...
		$this->form = nl4wp_get_form( $this->preview_id );
	}
	...
	public function set_page_title( $title ) {
		...
		return __( 'Form preview', 'newsletter-for-wp' );
	}
